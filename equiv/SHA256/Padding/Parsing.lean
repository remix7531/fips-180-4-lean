...
private def parseBytes (bs : ByteArray) : Array (Array (BitVec 32)) :=
...
private theorem parseBytes_eq_parse (bs : ByteArray) (hMod : bs.size % 64 = 0) :
  ...
  · intro i hi1 hi2
    ...
    apply Array.ext
    · simp [Array.size_map, List.toArray, List.length_map, List.length_range,
            toSpecBlock, Array.size_map, Vector.size_toArray]
    · intro j hj1 hj2
      have hjLt : j < 16 := by
        simp [List.size_toArray, List.length_map, List.length_range] at hj2; exact hj2
      have hLHS := toSpecBlock_toU32s_index
        (Vector.ofFn fun (k : Fin 64) => bs.get! (i * 64 + k.val)) ⟨j, hjLt⟩
      have hSizeLHS : j < (toSpecBlock (Impl.toU32s
          (Vector.ofFn fun (k : Fin 64) => bs.get! (i * 64 + k.val)))).size := by
        simp [toSpecBlock, Array.size_map, Vector.size_toArray]
        omega
      rw [show (toSpecBlock _)[j] = (toSpecBlock _)[j]! from
      ...
      have hSizeBs : i * 64 + 4 * j + 4 ≤ bs.size := by
        have hMod' : bs.size = 64 * (bs.size / 64) := by omega
      ...
      have hRhsList :
          ...
          [bs.data.toList[i * 64 + 4 * j]'(by rw [Array.length_toList, hData]; omega),
           bs.data.toList[i * 64 + 4 * j + 1]'(by rw [Array.length_toList, hData]; omega),
           bs.data.toList[i * 64 + 4 * j + 2]'(by rw [Array.length_toList, hData]; omega),
           bs.data.toList[i * 64 + 4 * j + 3]'(by rw [Array.length_toList, hData]; omega)] := by
        rw [List.drop_take, List.take_take]
        rw [Nat.min_eq_left (by omega : 4 ≤ 64 - 4 * j)]
        rw [List.drop_drop]
      ...
      have h0 : i * 64 + (4 * j + 0) < bs.size := by omega
      have h1 : i * 64 + (4 * j + 1) < bs.size := by omega
      have h2 : i * 64 + (4 * j + 2) < bs.size := by omega
      have h3 : i * 64 + (4 * j + 3) < bs.size := by omega
